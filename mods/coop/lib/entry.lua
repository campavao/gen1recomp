-- A slot-scrub entry screen, for the two things co-op needs typed: a room
-- code and a LAN address.
--
-- It exists because the Gen 1 naming grid has no digits (letters and
-- punctuation only), so NamingScreen physically cannot enter either one.
-- src/link/CodeEntry.lua already owns this interaction for the vanilla link
-- menu; this is the screen around it, drawn to match
-- LinkState's own code entry so the two do not look like different games.
--
...
-- a dotted IPv4 fits in 15 slots; trailing blanks are trimmed on confirm so
-- "10.0.0.5" does not have to be padded out by hand
Entry.ADDRESS = { charset = "0123456789. ", length = 15 }
...
function Entry:draw()
  ...
  -- 15 address slots do not fit at the code screen's 16px pitch, so the
  -- pitch follows the slot count and the row stays centred either way
  local n = self.state.length
  local pitch = n > 8 and 10 or 16
