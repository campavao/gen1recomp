...
--   4. The fog: a forced shrink is announced on the ticker, and the rung
--      it moves names the POKeMON it evolved -- CATERPIE at Lv5 cannot
--      stay one at Lv15 -- with no box at any point.
--   5. POK-199: sitting in the START menu is not immunity.  A bot planted
...
return function(game)
  ...
  game.save.party = { Pokemon.new(game.data, "CATERPIE", 5) }
  ...
  U.tap(game, "a") U.wait(15)
  if #game.save.party ~= 2 then return C.fail("A did not take the ball") end
  if game.stack:top() ~= C.ow() then
  ...
  if not E.debugRoundFog(1) then return C.fail("debugRoundFog refused") end
  local moved = false
  for _ = 1, 200 do
  ...
  if game.save.party[1].species == "CATERPIE" then return C.fail("CATERPIE did not evolve at the rung") end
  U.log(("TICKER: the shrink and the rung were news, not boxes: %q, %s")
    :format((evoLine:gsub("\n", " ")), game.save.party[1].species))

  -- ------- 5: POK-199, from the START menu
  E.setFog(600)
